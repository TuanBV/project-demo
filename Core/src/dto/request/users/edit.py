...
class UserEditRequest(BaseModel):
  ...
  birthday: str = Field(..., title="Birth day", pattern="^(\\d{4})\\/(0[1-9]|1[0-2])\\/(0[1-9]|[1-2][0-9]|3[0-1])$", min_length=1)
  registered_date: Optional[str] = Field(None, title="Register date", pattern="^(\\d{4})\\/(0[1-9]|1[0-2])\\/(0[1-9]|[1-2][0-9]|3[0-1])$")
  ...
  date_issued_identification: str = Field(..., title="Date issued identification", pattern="^(\\d{4})\\/(0[1-9]|1[0-2])\\/(0[1-9]|[1-2][0-9]|3[0-1])$")
  telephone_no: Optional[str] = Field(None, title="Telephone number", pattern="^0(\\d{9,10})$|^$")
  ...
  def check_email(cls, email, values):
    ...
    user = user_repo.check_email(email, values["employee_code"])
  ...
  def check_identification_number(cls, identification_number, values):
    # Get user by employee code
    user = user_repo.check_identification_number(identification_number, values["employee_code"])
    # Employee code exit
    if not user:
