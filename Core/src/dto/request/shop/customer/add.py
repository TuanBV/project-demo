...
class AddCustomerRequest(BaseModel):
  ...
  password: str = Field(..., title="Password", regex="(?=.{8,})((?=.*\\d)(?=.*[a-z])(?=.*[A-Z])|(?=.*\\d)" \
      "(?=.*[a-zA-Z])(?=.*[^A-Za-z0-9])" \
      "|(?=.*[a-z])(?=.*[A-Z])(?=.*[^A-Za-z0-9])).*", min_length=8, max_length=64)
  ...
  def shop_exist(cls, email):
    ...
    flag_check = shop_repository.check_customer_exist(email, shop_no)
...
class EmailRequest(BaseModel):
  ...
  mail_address: str = Field(..., title="Mail Address", regex="^[\\w.!#$%&’*+\\/=?^`{|}-]+@([\\w-]+\\.)+[\\w-]{2,}$", min_length=1, max_length=256)

  @validator("mail_address")
  def shop_exist(cls, mail_address, values):
    # Get information from context
    shop = context.user.value
    ...
    # mode is "register" or "edit"
    flag_check = shop_repository.check_customer_exist(mail_address, shop_no, values["customer_no"])

    # Check flag to raise error
    if not flag_check:
      raise CrmException(message=ERR_MESSAGE.ERRMSG0027)
