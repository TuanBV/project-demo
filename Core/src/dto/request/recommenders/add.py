...
class AddRecommenderRequest(BaseModel):
  """
  Add recommender request model
  """
  fullname: str = Field(..., title="Full name", min_length=1, max_length=50)
  email: str = Field(default="admin@saishunkan.vn", title="Email", max_length=256, pattern=r"^[\w.!#$%&’*+\/=?^`{|}-]+@([\w-]+\.)+[\w-]{2,}$")
  ...
  telephone_no: Optional[str] = Field(None, title="Telephone number", pattern=REGEX_TELEPHONE_NO)
