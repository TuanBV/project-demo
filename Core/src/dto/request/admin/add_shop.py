...
container = Container()
service_owner_repository = container.service_owner_repository()
class Shop(BaseModel):
  ...
  first_name_kana: Optional[str] = Field(..., title="First Name Katakana", min_length=1, max_length=20, regex=r"^([ァ-ヴ]|ー)+$")
  last_name_kana: Optional[str] = Field(..., title="Last Name Katakana", min_length=1, max_length=20, regex=r"^([ァ-ヴ]|ー)+$")
  ...
  note: str = Field(None, title="Note")
  payment_method_code: PaymentMethod = Field(title="Payment method")
  days_for_printing: str = Field(..., title="Time print confirm", regex="^[0-9]{1,4}$", min_length=1, max_length=4)
  services_content: str = Field(None, title="Services content")
...
class Account(BaseModel):
  ...
  @validator("email")
  def email_exist(cls, email, values):
    flag_check = service_owner_repository.check_email_exist(email)
    # Check role to set message error
    if values["role_kbn"] == UserRole.SHOP_STAFF.value:
      messages = ERR_MESSAGE.ERRMSG0050
    elif values["role_kbn"] == UserRole.SHOP_MANAGER.value:
      messages = ERR_MESSAGE.ERRMSG0049
    elif values["role_kbn"] == UserRole.SHOP_OWNER.value:
      messages = ERR_MESSAGE.ERRMSG0048
