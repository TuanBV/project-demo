...
const addCategory = async () => {
  ...
  if (res) {
    ...
    childCategory.value.isModalCategory = false
    childCategory.value.categoryId = ''
  }
}
const updateCategory = async () => {
  const isValid = validate(CategorySchema, category.value)
  if (!isValid) return
  const res = await categoryService.update(childCategory.value.categoryId, category.value)
  if (res) {
...
<template>
  <div
    v-if="childCategory.isModalCategory"
    class="fixed left-0 top-0 z-[999] flex h-full w-[100vw] items-center justify-center bg-black bg-opacity-35"
    @click.self="childCategory.isModalCategory = !childCategory.isModalCategory"
  >
    <div
    ...
    >
      <div class="mb-5 flex items-start justify-between border-b pb-2">
        ...
        <button class="float-end" @click="childCategory.isModalCategory = false">
