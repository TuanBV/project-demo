...
const filterScore = (dataSearch, element, item, flagValid) => {
  if (
    dataSearch.advance[element] !== '' &&
    parseInt(item[element], 10) < parseInt(dataSearch.advance[element], 10)
  ) {
    flagValid = false
  }
    ...
    if (flagValid && 'advance' in dataClone) {
      Object.keys(dataClone.advance).forEach(element => {
        ...
        if (element.includes('date_interview_from')) {
          ...
          dataClone.advance[element] !== '' &&
          item[element] !== dataClone.advance[element]
        ) {
          flagValid = false
        }
      })
...
export { searchItems, setDataFile, convertToTime, checkEvaluate, previewCV }
